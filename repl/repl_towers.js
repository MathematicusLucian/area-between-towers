...
function maxArea(height) {
     ...
     */
        var buildingsAhead = determineBuildingsAhead(height, iXAxis);
        // Loop -> Each Tower (Left to Right)
        buildingsAhead.forEach(function (jValue, jXAxis) {
            var areaAtCurrentIndices = calculateLargestAreaForIndex(iValue, jValue, jXAxis);
            /**
 ...
 */
function calculateLargestAreaForIndex(iValue, jValue, jXAxis) {
    /**
     * We need the X-axis value, if we are to calculate the area
     */
    var lowestTower = determineMaxHeightForPairs(iValue, jValue);
    /**
     * Determine area:
     * Multiply:
 ...
 */
function determineMaxHeightForPairs(iValue, jValue) {
    /**
 ...
 */
function determineBuildingsAhead(height, iXAxis) {
    /**
 ...
 * @param {number[]} height a list of tower heights
 */
function validateInputs(height) {
    if (height.length < 2 || height.length > 105)
        outOfRange();
    if (areTowerHeightsOutOfRange(height))
        outOfRange();
}
/**
 ...
 * @param {number[]} height a list of tower heights
 */
function areTowerHeightsOutOfRange(height) {
